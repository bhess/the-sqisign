...
ibq_vec_4_copy_ibz(ibq_vec_4_t *vec, const ibz_t *coeff0, const ibz_t *coeff1, const ibz_t *coeff2, const ibz_t *coeff3)
{
    ...
    ibz_init(&one);
    ibz_set(&one, 1);
    ibq_set(&(vec->v[0]), coeff0, &one);
    ibq_set(&(vec->v[1]), coeff1, &one);
    ibq_set(&(vec->v[2]), coeff2, &one);
    ibq_set(&(vec->v[3]), coeff3, &one);
    ibz_finalize(&one);
}
...
quat_lll_verify(const ibz_mat_4x4_t *mat, const ibq_t *delta, const ibq_t *eta, const quat_alg_t *alg)
{
    ...
    for (int i = 1; i < 4; i++) {
        ...
        quat_lll_bilinear(&norm, &(orthogonalised_transposed.m[i - 1]), &(orthogonalised_transposed.m[i - 1]), &(alg->p));
        ibq_inv(&tmp, &norm);
        ibq_mul(&mu, &b, &tmp);
        // tmp is mu^2
        ibq_mul(&tmp, &mu, &mu);
