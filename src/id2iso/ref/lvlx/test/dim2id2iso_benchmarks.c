...
// Scalar multiplication [x]P + [y]Q where x and y are stored inside an ibz_vec_2_t [x, y] and P, Q \in E[2^f]
void
...
// Compute tate pairing of a basis
static inline void
tate_basis(fp2_t *out, int e, const ec_basis_t *basis, const ec_curve_t *curve)
{
    ...
    ec_copy_curve(&ec, curve);
    pairing_reduced_tate(out, e, &basis->P, &basis->Q, &wi, &ec);
}

int
id2iso_benchmarks_input_generation(quat_ideal_t *ideals, int bitsize, int iterations)
{
    int randret = 1;
    ibz_t norm;
    ibz_init(&norm);
    for (int i = 0; i < iterations; i++) {
        randret = randret && ibz_generate_random_prime(&(norm), 0, bitsize);
        if (!randret)
            return (!randret);
        randret = randret && quat_random_ideal_O0_given_prime_norm(&(ideals[i]), &norm, &PRNG_default_domain);
